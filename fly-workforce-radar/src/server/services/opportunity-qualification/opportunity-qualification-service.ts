...
// Phase 2Q. Manager-authorized 5th tracked opportunity (see PHASE-2Q-TRILLIUM-TRACKING).
// One conservative Seed aggregating both real Trillium Amarillo postings (Journeyman
// #791374, Apprentice #791431) -- same employer, same location, same recruiter,
// contemporaneous captures -- not two opportunities. buyerCandidate stays null: the
// fact's own "Trillium Construction Services" is Trillium's OWN staffing-company role
// (already carried on `company`, matching how qual-permian carries Strike as employer
// without inventing a buyer), not a named end client, and no end client is stated
// anywhere in the real evidence. af01Category preserves the fact's own
// THIRD_PARTY_RECRUITING_ACCEPTED classification as an unverified CANDIDATE only --
// af01VerificationState is "MISSING", never "VERIFIED": Trillium's own posting does not
// constitute verified manpower acceptance for an unidentified end buyer (manager policy
// decision). routeGrade stays null: this codebase never auto-assigns a grade to real
// evidence; only human review can.
{id:"qual-amarillo",context:"Trillium Amarillo journeyman and apprentice electrician demand",market:"Texas Panhandle",currentDemand:true,electricalRoles:["Licensed Journeyman Electrician","TDLR Apprentice Electrician"],demandSources:["trillium-amarillo-journeyman-791374","trillium-amarillo-apprentice-791431"],company:"Trillium Construction Services",canonicalCompanyStatus:"RESOLVED",project:null,projectStatus:"UNKNOWN",buyerCandidate:null,buyerVerificationStatus:"MISSING",companyRole:"THIRD-PARTY STAFFING RECRUITER; MANPOWER_BUYER (end client) unknown",af01Candidate:"Trillium's own posting describes a third-party W-2 staffing assignment",af01Category:"THIRD_PARTY_RECRUITING_ACCEPTED",af01VerificationState:"UNVERIFIED",contactPerson:"Roberto Venegas",contactFunction:"RECRUITER",contactRoute:"800-778-8907",routeType:"RECRUITER_PHONE",routeGrade:null,routeVerificationState:"UNVERIFIED",compensation:"Journeyman up to $43/hr; Apprentice $20.35-$43/hr wage progression",overtime:null,perDiem:"$100/day (both postings)",headcount:null,schedule:"Journeyman: Monday-Friday, 7am-3:30pm; Apprentice: 40-50 hours/week, first shift, Monday-Friday with some Saturdays",duration:"Journeyman: approximately 18 months of steady work; Apprentice: 4-year Apprentice Program",firstSeen:at,lastSeen:at,staleAfter:new Date("2026-09-27T00:00:00Z"),evidenceIds:[evidence("trillium-amarillo-791374"),evidence("trillium-amarillo-791431")],claimIds:[],sourceUrls:["https://trilliumstaffing.com/jobs/job/791374-licensed-journeyman-electrician-per-diem-offered-amarillo-texas.html","https://trilliumstaffing.com/jobs/job/791431-tdlr-apprentice-electrician-per-diem-offered-amarillo-texas.html"],conflicts:[],humanReviewRequirements:["Determine whether an end-client manpower buyer exists behind Trillium's staffing role, or whether only a HOT-B intelligence-only path applies","Assign or reject a route grade for Roberto Venegas's recruiter-phone contact","Determine whether Trillium's own third-party-recruiting classification can ever satisfy MANPOWER_ACCEPTANCE_REQUIRED without a named end-client acceptance"]}
];
...
// real DB-backed opportunity-service.ts's graph() (see opportunity-graph.test.ts):
// a gap is absent only when the underlying state is genuinely current+verified as
...
const gapsFor=(x:Seed):OpportunityGraph["gaps"]=>[
  ...
  ...(x.staleAfter&&x.staleAfter<=at?["STALE_DEMAND"as const]:[]),
  ...(x.company?[]:["MISSING_COMPANY"as const]),
  ...(x.buyerCandidate?[]:["MISSING_COMPANY_ROLE"as const]),
  ...(x.project?[]:["MISSING_PROJECT"as const]),
  ...(acceptanceCurrentlyVerified(x)?[]:["MISSING_MANPOWER_ACCEPTANCE"as const]),
  ...(x.contactPerson?[]:["MISSING_CONTACT"as const]),
  ...(routeCurrentlyActionable(x)?[]:["MISSING_ACTIONABLE_ROUTE"as const]),
  ...(x.conflicts.length?["CONFLICTING_EVIDENCE"as const]:[]),
];
